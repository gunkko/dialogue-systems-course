...
#
# Общие закономерности (без результатов выполненного эксперимента):
# 1. **Temperature**: Низкая температура уменьшает случайность выбора слов,
#    высокая — увеличивает разнообразие и непредсказуемость формулировок.
# 2. **Max Tokens**: Малый лимит может обрывать ответ до завершения мысли.
#    Больший лимит позволяет дать более подробный и полный ответ.
# 3. **Top P**: При 0.1 выбор ограничен наиболее вероятными токенами,
#    при 1.0 доступны все варианты, поэтому формулировки могут быть разнообразнее.
# 4. **System Prompt**: Роль эксперта подходит для технических объяснений,
#    учителя — для простых объяснений детям, поэта — для творческой подачи.

# %%
# Место для ваших дополнительных экспериментов

# %% [markdown]
# ## Дополнительные эксперименты: учёба, путешествия и наука

# %%
print("=" * 60)
print("ДОПОЛНИТЕЛЬНЫЕ ЭКСПЕРИМЕНТЫ: УЧЁБА, ПУТЕШЕСТВИЯ И НАУКА")
print("=" * 60)

# 1. Планирование подготовки к экзамену
study_plan = chat_with_params(
    "Составь план подготовки к экзамену по Python на три дня. "
    "Темы: циклы, функции и списки. На занятия есть один час в день. "
    "Для каждого дня укажи тему и одно практическое задание. "
    "Ответь нумерованным списком.",
    temperature=0.3,
    max_tokens=200,
)
print("\n--- 1. План подготовки к экзамену ---")
print(study_plan)

# 2. Уточнение предпочтений путешественника
travel_questions = chat_with_params(
    "Путешественник хочет провести выходные в незнакомом городе, "
    "интересуется архитектурой и прогулками. Составь три уточняющих "
    "вопроса, которые нужно задать перед составлением маршрута. "
    "Ответь нумерованным списком, сам маршрут пока не предлагай.",
    temperature=0.5,
    max_tokens=150,
)
print("\n--- 2. Уточнение предпочтений путешественника ---")
print(travel_questions)

# 3. Объяснение научного понятия простыми словами
science_explanation = chat_with_params(
    "Объясни ребёнку десяти лет, как происходит круговорот воды в природе. "
    "Используй пример с лужей после дождя. Ответь в четырёх коротких "
    "предложениях и избегай сложных терминов.",
    temperature=0.4,
    max_tokens=150,
)
print("\n--- 3. Круговорот воды простыми словами ---")
